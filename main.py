...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Как подключить :gear:":
        if user_dat.trial_subscription == False:
            ...
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Инструкция для iPhone"), url="https://telegra.ph/"))
            Butt_how_to.add(
                types.InlineKeyboardButton(e.emojize("Инструкция для Android"), url="https://telegra.ph/"))
            Butt_how_to.add(
